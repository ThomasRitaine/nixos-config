...
{
  options.garageAutoCapacity = lib.mkOption {
    type = lib.types.str;
    default = "175G";
    description = "Default capacity assigned to Garage nodes by the auto-provision script.";
  };
  ...
  config = {
    ...
    # Automatically register the node to the cluster
    systemd.services.garage-auto-provision = {
      ...
      script = ''
        ...
        if [ -n "$LOCAL_NODE_ID" ]; then
          # Stage the defined capacity limit and assign a default zone
          $GARAGE_BIN layout assign -c ${config.garageAutoCapacity} -z default "$LOCAL_NODE_ID" || true
        fi
      '';
    };
